import { getAllDocs, getOneDoc, updateDoc } from "../../../_services/global";
import { checkPlanName } from "../../../_services/plan/planService";
...
export const getPlanController = catchAsync(async (request, response, next) => {
    const plan = await getOneDoc(Plan, request.params.planId);
...
export const updatePlanController = catchAsync(async (request, response, next) => {
    ...
    if(!planName?.trim() && !price && !discount && !features.length && !quota)
        return next(new AppError(400, "no data was provided to update"));

    const updatedPlan = await updateDoc(Plan, request.params.planId, {});
...
export const getPlansStats = catchAsync(async (request, response, next) => {
    // TODO: returns this/last/total stats
});