...
async function registerEventConsumers() {
  ...
  userDeletedRegister({ receiver: novuDeleteSubscriber, queueName: "user-deleted-queue-novu", requeue: true });
  userDeletedRegister({ receiver: deleteUserFromCache, queueName: "user-deleted-queue-redis", requeue: false });
  userDeletedRegister({ receiver: deleteMultipleCredentials, queueName: "user-deleted-queue-credentials-collection", requeue: true });
}
