...
eventBus.ofType<UserUpdatedEvent>("user-updated").subscribe((event) => {
  ...
  const safeNovuTrigger = ResultAsync.fromThrowable(
    () => novuUpdateSubscriber(user.id, user),
    () => new Error("couldn't update the Novu subscriber")
  );
  safeNovuTrigger();
});
...
eventBus.ofMultipleTypes<[UserUpdatedEvent, UserLoggedInEvent, UserCredentialsUpdatedEvent]>(["user-updated", "user-loggedIn", "userCredentials-updated"]).subscribe((event) => {
  ...
  const safeUpdateRedisCache = ResultAsync.fromThrowable(
    () => cacheUser(user, emailConfirmed),
    () => new Error("couldn't update Redis cache")
  );
