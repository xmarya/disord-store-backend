import { DigitalProduct, PhysicalProduct, ProductDataBody } from "../../_Types/ProductNew";
...
export const validateProductBody = catchAsync(async (request, response, next)=> {
    ...
    if(!name?.trim() || !productType?.trim() || !price || !description?.trim()) return next(new AppError(400, "Please provide all necessary product data and specify the productType to be either physical or digital."));
